...
每次最多请求 200 条热门视频和 100 条全站排行榜视频，去重后同步。翻页请求间隔 1 秒；B站以 -352 拒绝后续热门页时，立即停止翻页，用已获取页面尝试同步并在页面提示本次仅为部分热门列表。第一页被拒或没有可用热门数据时，任务失败并保留云端旧数据；仅排行榜失败时仍更新热门数据，页面明确提示排行榜不可用，不冒用历史排名。不绕过访问限制、不自动重试。
